...
//      - JPA는 기본 생성자가 반드시 필요, 외부에서의 임의 생성 방지
@Getter
@Setter
public class User implements UserDetails {
    ...
    // EAGER(즉시로딩): 전체 데이터와 해당 필드가 동시에 사용되는 경우
    @JoinTable(
            name = "user_roles", // 중간 테이블 명
...
//        return List.of(new SimpleGrantedAuthority("user"));
        ...
        // 사용자의 Role을 Spring Security가 인식 할 수 있도록 변환
        // EX) "USER" -> "ROLE_USER"
        return roles.stream()
                // 로그인 후 인증된 사용자의 권한을 Spring Security에 전달
                // : hsaRole() 방식과 호환되기 위한 설정
