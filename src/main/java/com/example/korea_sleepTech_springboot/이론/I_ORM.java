...
public class I_ORM {
    ...
    // : 객체와 관계형 DB간의 데이터를 1:1로 매핑해주는 기술
    ...
    // - 반복적인 SQL 작성 없이 데이터 조작 가능 (DBMS에 독립정)
    // <단점>: 학습 곡선 존재, 복잡한 쿼리 작성 어려움

    // 2. ORM 동작 원리
    // : 각 테이블은 클래스에, 테이블의 각 행(Row)은 객체에 매핑
    // - Entity: DB의 테이블에 매핑되는 클래스 (@Entity)
    // - Field: DB의 열(Column)에 해당하는 필드

    // 3. JPA (Java Persistence API)
    // : ORM 중 하나
    // - Java 애플리케이션에서 관계형 DB를 쉽게 다를 수 있는 ORM 표준 기술
    ...
    */
    ...
    // 2. ID
    // : 해당 필드를 DB 테이블의 'PK(기본키)'로 지정
    // +) 옵션 없이 사용 가능하나, 다른 애너테이션과 함께 기본키 생성 방식이나 타입 지정 가능
    //      >> 주로 GeneratedValue와 함께 사용
    //          (기본키 자동 생성 전략 설정 - strategy 옵션/ GenerationType.IDENTITY(auto_increment)

    // @Column
    // : 필드를 특정 테이블의 열과 매핑
    // - 생략시 기본으로 필드 이름이 열 이름으로 사용
    // +) 옵션
    //      - name 옵션: 열 이름 지정
