...
public class B_HTTP02 {
    /*
        ...
        cf) HTTP 기본 포트: 80
            HTTPS 기본 포트: 443
            +) Tomcat은 주로 8080포트를 사용

        1. HTTP 메서드(Method)
        : "요청이 어떤 목적을 가지고 있는지" 알려주는 방법

        1) GET(가져오기)
            : 서버로부터 데이터를 가져올 것을 요청

        2) POST(생성하기)
            : 서버로부터 데이터를 저장할 것을 요청

        3) PUT(수정하기)
            : 서버로부터 데이터를 수정할 것을 요청

        4) DELETE(삭제하기)
            : 서버로부터 데이터를 삭제할 것을 요청
        ...
        4) 401 Unauthorized: 인증 실패 (EX. 권한이 없는 요청)
        5) 403 Forbidden: 권한 없음 (EX. 회사 내 보안 문서는 권한 없이 열람 불가)
        6) 404 Not Found: 요청 자체를 찾을 수 없음 (EX. 해당하는 요청이 아예 없음)
