    ...
    /*
        ...
        1. @RequestParam
            : 클라이언트가 보낸 URL 쿼리 스트링 또는 폼 데이터를
                , 컨트롤러 메서드의 파라미터로 바인딩 할 때 사용
            - 주로 GET 요청에 많이 사용

            >> 간단한 검색 조건(URL 뒤에 간단한 데이터 전달), 필터링 & 페이징 가능, 보안이 크게 중요하지 않은 데이터
                : URL에 노출되기 때문에 민감한 정보는 비추천

            cf) 옵션 정리
            1) @RequestParam(required = ture): 값이 없으면 오류<기본>
            2) @RequestParam(required = false): 값이 없어도 허용 (null 허용)
            2) @RequestParam(defaultValue = "값"): 기본값 설정
        ...
        2. @RequestBody
            ...
            - DTO 객체는 반드시 getter/setter 또는 @Data가 있어야 함
