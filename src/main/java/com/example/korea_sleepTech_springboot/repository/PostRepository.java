...
//    List<D_Post> findByAuthor(String author);
    ...
    // cf) @Query 애너테이션
    //      : 직접 JPQL이나 SQL을 작성해서 쿼리를 실행 시켜주는 애너테이션
...
//    List<D_Post> findByTitleContaining(String title);
    List<D_Post> findByTitleIgnoreCaseContaining(String title);

//    @Query("SELECT p FROM D_Post p WHERE p.title LIKE %:keword%")
//    List<D_Post> searchByTitle(@Param("keyword") String keyword);
    ...
    @Query(value = """
                ...
                Like %:keyword%
            """, nativeQuery = true)
    List<D_Post> searchPostByKeywordWithComment(@Param("keyword") String keyword);
}
