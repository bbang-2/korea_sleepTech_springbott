...
public class BookService {
    ...
    public ResponseDto<BookResponseDto> createBook(BookCreateRequestDto requestDto) {
        try {
            C_Book newBook = new C_Book(null, requestDto.getWriter(), requestDto.getTitle()
    ...
    public List<BookResponseDto> getAllBooks() {
        List<BookResponseDto> responseDto = null;
        try {
            List<C_Book> books = bookRepository.findAll();

            responseDto = books.stream()
    ...
    public BookResponseDto updateBook(Long id, BookUpdateRequestDto dto) {
        ...
        try {
            ...
            C_Book updateBook = bookRepository.save(book);

            responseDto = new BookResponseDto(
                    updateBook.getWriter(),
                    updateBook.getTitle(),
                    updateBook.getCategory()
            );
    ...
    public ResponseDto<List<BookResponseDto>> getBooksByTitleContaining(String keyword) {
        ...
        try {
            ...
            return ResponseDto.setSuccess("책검색을 정상적으로 완료하였습니다.", books);
        } catch(Exception e) {
            e.printStackTrace();
    ...
    public ResponseDto<List<BookResponseDto>> getgetBooksByCategory(C_Category category) {
        List<BookResponseDto> books = null;
        try {
            ...
            e.printStackTrace();
            return ResponseDto.setFailed("카테고리별 책을 검색하는동안 문제가 발생하였습니다.");
        }
    }

    public ResponseDto<List<BookResponseDto>> getBookByCategoryAndWriter(C_Category category, String writer) {
        List<BookResponseDto> books = null;
