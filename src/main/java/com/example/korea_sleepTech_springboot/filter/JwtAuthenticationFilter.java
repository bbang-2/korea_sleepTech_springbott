...
/*
    ...
    cf) Spring Security의 OncePerRequestFilter를 상속받아 매 요청시 실행
 */
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    /*
        doFilterInternal
        : 요청 시 헤더의 Authorizatiom에서 JWT 토큰을 추출
    ...
    protected void doFilterInternal(
        ...
        try {
            ...
            // JWT 토큰이 유효한 경우 해당 토큰에서 사용자 ID 추출
            String username = jwtProvider.getUsernameFromJwt(token);
            Set<String> roles = jwtProvider.getRolesFromJwt(token);

            // 추출한 사용자 ID를 SecurityContext에 인증 정보 저장
    ...
    /*
        ...
        - SetAuthenticationContext()는 요청에서 username값을 SecurityContext에 인증 정보로 설정
            : UsernamePasswordAuthenticationToken을 생성하고
                , 해당 토큰에 username 값을 넣어 인증 정보로 등록

        >> Spring Security는 SecurityContextHolder에 잇는 인증 정보를 자동으로
    ...
    private void SetAuthenticationContext(HttpServletRequest request, String username, Set<String> roles) {
        ...
        List<GrantedAuthority> authorities = roles.stream()
                ...
                // , 토큰에 담긴 roles 정보를 hasRole("")과 매칭 시키기 위함
