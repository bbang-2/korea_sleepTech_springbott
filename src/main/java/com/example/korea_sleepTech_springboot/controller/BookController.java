...
//    }
    ...
    public ResponseEntity<ResponseDto<List<BookResponseDto>>> getBooksByTitleContaining(
            ...
            // >> 키의 변수에 값이 할당
            // >> localhost:8080/books/search/title?keyword=cxx
    ...
    public ResponseEntity<ResponseDto<List<BookResponseDto>>> getBooksByCategory(
        ...
        try {
        ResponseDto<List<BookResponseDto>> books = bookService.getgetBooksByCategory(category);
        return ResponseEntity.status(HttpStatus.OK).body(books);
    ...
    // 제목 검색) QueryParameter 사용
    //          >> GET /books/search/title?keyword=콩
    // : 제목에 특정 단어가 포함되어있는 책들을 검색
    // > 정확한 자원을 가르키는 것이 아니라 필터 조건을 제시하는 형태

    // 카테고리별 조회) Path Variable 사용
    //          >> GET /books/category/NOVEL
    // : Category 값이 어떤 고정된 그룹 (NOVEL, ESSAY, POEM 등) 중 하나
    // - "A라는 카테고리에 속한 책 리스트"라는 명확한 리소스를 가르키는 URI로 간주

    // 3) 카테고리와 작성자별 책 조회
    // : 복합 조건 검색은 보통 Query Parameter를 조합해서 처리
    @GetMapping("/search/category-writer")
    public ResponseEntity<ResponseDto<List<BookResponseDto>>> getBookByCategoryAndWriter(
        ...
        try {
            ResponseDto<List<BookResponseDto>> books = bookService.getBookByCategoryAndWriter(category, writer);
            return ResponseEntity.status(HttpStatus.OK).body(books);
        } catch (Exception e) {
            e.printStackTrace();
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(null);
