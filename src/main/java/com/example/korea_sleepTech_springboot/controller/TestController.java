...
//@Controller: 웹 요청을 처리하는 클래스 임을 명시 (반환되는 데이터타입이 유연함 - JSP, Thymeleaf 등)
@RestController // @Controller + @ResponseBody 합친 것 (데ㅔ이터를 반환할 때 - JSON 등)
...
public class TestController {
    ...
    @GetMapping("/{id}") // @메서드Mapping(추가 Ur 지정) // GET + localhost:8080/test/숫자값
    public A_TestEntity getTestById(@PathVariable Long id) { // @PathVariable: URL 경로에서 변수값을 포함하여 전달
        A_TestEntity test = testService.getTestById(id);
        return test;
    }

    @PostMapping
    public A_TestEntity creatTest(@RequestBody A_TestEntity ATestEntity) {
    ...
    @DeleteMapping
    public A_TestEntity deleteTest(@PathVariable Long id) {
        return null;
    }
}
